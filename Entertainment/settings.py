...
SECRET_KEY = 'fddhde^3(6)e*40@d*%(jh%w&5er*ln@d^j*#%gn5*-qs#yr@0'
...
MIDDLEWARE = [
    ...
    # 'Main_User.network.TestingMiddlewareMixin',
]
...
DATA_UPLOAD_MAX_MEMORY_SIZE = 20971520  # 默认设置为20M
...
LOGGING = {
    ...
    'handlers': {
        ...
        'default': {
            ...
            'filename': os.path.join(BASE_LOG_DIR, "django_info.log"),  # 日志文件
        ...
        'error': {
            'level': 'ERROR',
            'class': 'logging.handlers.RotatingFileHandler',  # 保存到文件，自动切
            'filename': os.path.join(BASE_LOG_DIR, "django_err.log"),  # 日志文件
            'maxBytes': 1024 * 1024 * 50,  # 日志大小 50M
            'backupCount': 5,
            'formatter': 'standard',
            'encoding': 'utf-8',
